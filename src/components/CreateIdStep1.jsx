...
const CreateIdStep1 = ({ onClose, onClick, title, subtitle, logo }) => {
  ...
  return (
    <div className="bgt-blue3 text-white font-medium text-[15px] rounded-2xl shadow-md  w-full relative overflow-hidden  max-w-3xl">
      ...
      <div className="flex justify-center mb-2">
        <img src={logo} alt="Logo" className="w-[71px] h-[71px] object-contain" />
      </div>

      <div className="mb-4">
        <h4 className="text-white text-center text-xl font-medium leading-tight">{title}</h4>
        <h4 className="text-white text-center text-xl font-medium leading-tight">{subtitle}</h4>
      </div>

      {/* Form */}
      <form className="flex flex-col gap-2 px-3 text-[15px] font-medium space-y-1 mb-5">
        <div>
          <label className="text-white">Username*</label>
          <input type="text" placeholder="Username" className="font-inter font-normal h-[45px] ct-black5 w-full rounded-[10px] px-3 py-2 bg-[var(--theme-grey5)] text-sm outline-none" />
        </div>
        ...
        <div className="mb-3 relative">
          ...
          />
          <button type="button" className="absolute right-3 top-[36px] text-gray-600" onClick={() => setShowPassword((prev) => !prev)}>
            {showPassword ? <FiEyeOff size={20} /> : <FiEye size={20} />}
          </button>
        </div>

        <button className="bgt-blue2 rounded-lg px-6 py-2.5 w-full t-shadow5" onClick={onClick} type="submit">
          Create Instant ID
        </button>
      </form>
    </div>
